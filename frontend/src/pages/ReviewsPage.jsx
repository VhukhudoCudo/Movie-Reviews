...
export default function ReviewsPage() {
  ...
  const fetchReviews = async () => {
    try {
      const res = await axios.get('https://movie-reviews-s63z.onrender.com/reviews');
      setReviews(res.data);
    } catch (err) {
      console.error(err);
    }
  ...
  const addReview = async () => {
    ...
    try {
      if (editing) {
        // Edit review
        await axios.put(`https://movie-reviews-s63z.onrender.com/reviews/${editing._id}`, { movie, review });
        setEditing(null); // Reset editing state
      } else {
        // Add new review
        await axios.post('https://movie-reviews-s63z.onrender.com/reviews', { movie, review });
      }
      setMovie('');
      setReview('');
      fetchReviews();
  ...
  const deleteReview = async (id) => {
    try {
      await axios.delete(`https://movie-reviews-s63z.onrender.com/reviews/${id}`);
      fetchReviews();
    } catch (err) {
